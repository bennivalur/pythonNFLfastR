...
def makegraphs(year_from, year_to,descr, cold,hot):
    ...
    with open(descr + '/main.txt', 'w') as file:
    ...
    ax.set_ylabel('Passing EPA in cold (<= ' + str(cold_limit) + '°F)', fontsize=16)
    ax.set_xlabel('Passing EPA in heat (>= ' + str(heat_limit) + '°F)', fontsize=16)
...
columns = ['game_id','posteam','play_type','epa','roof','temp','season','passer']
descr  = "passing_cold"
...
makegraphs(1999,2019,descr,40,80)